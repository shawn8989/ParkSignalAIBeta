...
actor AIAnalyzerService {
    ...
    /// The key configured in Settings (UserDefaults), falling back to Info.plist for development builds.
    ...
    static var isConfigured: Bool { configuredAPIKey != nil }
    ...
    func analyze(ocrText: String, apiKey: String? = nil) async throws -> (parsed: AIAnalysisResponse, rawJSON: String) {
        let key = apiKey ?? Self.configuredAPIKey
        guard let apiKey = key, !apiKey.isEmpty else {
            throw NSError(domain: "AIAnalyzerService", code: -1, userInfo: [NSLocalizedDescriptionKey: "No OpenAI API key configured. Add one in Settings to enable AI analysis."])
        }
